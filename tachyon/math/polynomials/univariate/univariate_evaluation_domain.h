...
class UnivariateEvaluationDomain : public EvaluationDomain<F, MaxDegree> {
 public:
  ...
  constexpr UnivariateEvaluationDomain(size_t size, uint32_t log_size_of_group)
    ...
    std::optional<F> inv = size_as_field_element_.Inverse();
    CHECK(inv);
    size_inv_ = std::move(*inv);
    ...
    inv = *group_gen_.Inverse();
    CHECK(inv);
    group_gen_inv_ = std::move(*inv);
  }
  ...
  constexpr std::unique_ptr<UnivariateEvaluationDomain> GetCoset(
    ...
    std::optional<F> inv = offset.Inverse();
    CHECK(inv);
    coset->offset_inv_ = std::move(*inv);
  ...
  constexpr std::vector<F> EvaluatePartialLagrangeCoefficients(
    ...
    if (z_h_at_tau.IsZero()) {
      ...
      //    = (Z_H(τ) * h * gᵢ * v₀⁻¹ * h⁻¹)⁻¹
      //    = (Z_H(τ) * gᵢ * v₀)⁻¹
      std::optional<F> l_sub_inv = (z_h_at_tau * omega_i).Inverse();
      CHECK(l_sub_inv);
      F l_i = std::move(*l_sub_inv) * t;
