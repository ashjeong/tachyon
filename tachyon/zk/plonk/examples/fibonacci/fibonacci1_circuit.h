...
struct Fibonacci1Config {
  ...
  Fibonacci1Config(std::array<AdviceColumnKey, 3> advice, Selector selector,
      ...
      : advice(std::move(advice)), selector(selector), instance(instance) {}

  Fibonacci1Config Clone() const {
    return Fibonacci1Config(advice, selector, instance);
  }
...
class Fibonacci1Chip {
 public:
  ...
  static Fibonacci1Config<F> Configure(ConstraintSystem<F>& meta) {
    ...
    return Fibonacci1Config<F>(std::move(advice), std::move(selector),
                               std::move(instance));
  }
