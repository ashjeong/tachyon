  ...
  static Fibonacci3Config<F> Configure(ConstraintSystem<F>& meta) {
    ...
    IsZeroConfig<F> a_equals_b = IsZeroChip<F>::Configure(
        ...
        [&a, &b](VirtualCells<F>& meta) {
          // Note(ashjeong): Logic is not implemented as an in-line
          // return, as Linux/Mac systems would result in different outcomes.
          // Further research into this phenomenon may be needed.
          auto x = (meta.QueryAdvice(a, Rotation::Cur()));
          auto y = (meta.QueryAdvice(b, Rotation::Cur()));
          return x - y;
        },
