...
class Value : public math::Field<Value<T>> {
 public:
  ...
  constexpr std::optional<Value> Inverse() const {
    ...
    if (UNLIKELY(!val_inv)) return std::nullopt;
    return Value::Known(*val_inv);
  }

  constexpr std::optional<Value*> InverseInPlace() {
    if (IsNone()) return this;
    if (UNLIKELY(!(*value_).InverseInPlace())) return std::nullopt;
    return this;
  }
