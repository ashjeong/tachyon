...
struct Poseidon2Config : public PoseidonConfigBase<Params> {
  ...
  constexpr static Poseidon2Config CreateCustom(
      const std::array<PrimeField, Params::Width>&
          internal_diagonal_minus_one) {
    constexpr Poseidon2ConfigEntry config_entry(
        Params::Rate, Params::Alpha, Params::FullRounds, Params::PartialRounds);
    Poseidon2Config ret = config_entry.ToPoseidon2Config<Params>();
    ret.internal_diagonal_minus_one = math::Vector<F>(Params::Width);
  ...
  constexpr static Poseidon2Config CreateCustom(
      const std::array<uint8_t, Params::Rate>& internal_shifts) {
    Poseidon2ConfigEntry config_entry(
        Params::Rate, Params::Alpha, Params::FullRounds, Params::PartialRounds);
    math::Matrix<F> ark;
 ...
 private:
  constexpr static Poseidon2Config CreateCustom(
    ...
    if constexpr (math::FiniteFieldTraits<F>::kIsPackedPrimeField) {
      ...
      for (size_t i = 1; i < Params::Width; ++i) {
        ret.internal_diagonal_minus_one[i] =
            F(uint32_t{1} << internal_shifts[i - 1]);
      }
    } else {
      ret.internal_shifts = math::Vector<uint8_t>(Params::Rate);
      for (size_t i = 0; i < Params::Rate; ++i) {
